...
- MySQL 애플리케이션 계정: `assignment_user`
...
CREATE USER 'assignment_user'@'localhost' IDENTIFIED BY '원하는비밀번호';

GRANT ALL PRIVILEGES ON assignment.* TO 'assignment_user'@'localhost';

FLUSH PRIVILEGES;
```

위 설정은 `assignment_user`가 `assignment` 데이터베이스에 대해서만 접근 권한을 갖도록 만든다.
...
아래 명령어로 `assignment_user` 계정 접속을 확인한다.

```bash
mysql -u assignment_user -p assignment
```
...
- User가 `assignment_user`인지 확인
- Password가 계정 생성 시 입력한 값과 같은지 확인
- Database가 `assignment`인지 확인
- `GRANT ALL PRIVILEGES ON assignment.* TO 'assignment_user'@'localhost';`를 실행했는지 확인
...
관리 작업은 `root` 계정으로 수행하고, Spring Boot와 DataGrip 연결에는 `assignment_user`를 사용한다.
