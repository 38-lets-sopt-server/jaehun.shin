...
| 심화 1 | Access Token 만료 시 클라이언트 401 처리 흐름 작성 | 이 문서의 "10. 클라이언트 401 처리 흐름"에 정리했다. |
...
이번 과제 구현 전에는 JWT 실습 흐름 일부는 있었지만, 비밀번호 평문 비교, 요청 body의 `userId` 신뢰, 로그아웃/블랙리스트 부재, 좋아요 API 부재, 소셜 로그인 부재라는 한계가 있었다.
...
sequenceDiagram
    ...
    UR-->>AS: User
    AS->>AS: password 평문 비교
    AS->>JS: Access/Refresh Token 발급
    AS->>RR: 기존 Refresh Token 삭제 후 새 토큰 저장
    AS-->>AC: TokenResponse
    AC-->>C: Access Token + Refresh Token

    C->>PC: POST /posts + body.userId
    PC->>PS: createPost(request)
    PS->>UR: findById(request.userId)
    UR-->>PS: User
    PS->>PR: Post 저장
    PR-->>PS: saved Post
    PS-->>PC: postId
...
구현 전 흐름의 문제점은 다음과 같다.

- 로그인은 DB의 `password`와 요청 password를 평문으로 비교했다.
- 게시글 작성자는 인증된 사용자 기준이 아니라 요청 body의 `userId`로 결정됐다.
- 게시글 수정/삭제에서 작성자 본인 여부를 검증하지 않았다.
- 좋아요 추가/취소 API가 없었다.
- 로그아웃 API와 Access Token 블랙리스트가 없어서 발급된 Access Token은 만료 전까지 계속 사용할 수 있었다.
- Kakao/Google 소셜 로그인 흐름이 없었다.
...
sequenceDiagram
    ...
    participant AC as AuthController
    participant AS as AuthService
    participant OC as OAuthUserInfoClient
    participant EXT as Kakao/Google
    participant UR as UserRepository
    participant SR as SocialAccountRepository
    participant RR as RefreshTokenRepository
    participant BR as AccessTokenBlacklistRepository
    participant PC as PostController
    participant PS as PostService
    participant LR as PostLikeRepository

    alt 일반 회원가입
        C->>AC: POST /api/v1/auth/signup
        AC->>AS: signup(nickname, email, password)
        AS->>AS: BCryptPasswordEncoder.encode(password)
        AS->>UR: 암호화된 password로 User 저장
        AS-->>AC: 회원 정보
        AC-->>C: 회원가입 성공
    end
    ...
    alt 일반 로그인
        ...
        AS->>RR: 기존 Refresh Token 삭제
        AS->>RR: 새 Refresh Token 저장
        AS-->>AC: Access Token + Refresh Token
        AC-->>C: 로그인 성공
    end

    alt Kakao/Google 소셜 로그인
        C->>AC: POST /api/v1/auth/oauth/{provider}
        AC->>AS: oauthLogin(provider, accessToken)
        AS->>OC: 외부 사용자 정보 조회
        OC->>EXT: UserInfo API 호출
        EXT-->>OC: providerUserId, email, nickname
        OC-->>AS: OAuthUserInfo
        AS->>SR: SocialAccount 조회
        alt 기존 소셜 계정 있음
            SR-->>AS: 연결된 User
        else 신규 또는 기존 이메일 회원
            AS->>UR: email 기준 User 조회 또는 생성
            AS->>SR: SocialAccount 연결 저장
        end
        AS->>RR: Refresh Token 저장
        AS-->>AC: Access Token + Refresh Token
        AC-->>C: 소셜 로그인 성공
    end

    C->>JF: 보호 API 요청 + Access Token
    JF->>BR: 블랙리스트 토큰 여부 확인
    JF->>JF: JWT 서명/만료 검증
    JF->>JF: SecurityContext에 memberId 저장
    JF->>PC: 요청 전달

    alt 게시글 작성/수정/삭제
        PC->>PS: 인증된 memberId로 게시글 처리
        PS->>PS: 작성자 본인 여부 검증
        PS-->>PC: 처리 결과
        PC-->>C: 성공 또는 403
    else 좋아요 추가/취소
        PC->>PS: like/unlike(postId, memberId)
        PS->>LR: 중복 없이 추가 또는 멱등 삭제
        LR-->>PS: likeCount
        PS-->>PC: postId, likeCount, likedByMe
        PC-->>C: 좋아요 결과
    end

    C->>AC: POST /api/v1/auth/logout + Access Token
    AC->>AS: logout(memberId, accessToken)
    AS->>RR: 해당 유저 Refresh Token 삭제
    AS->>BR: 현재 Access Token 블랙리스트 저장
    AC-->>C: 로그아웃 성공
```

구현 후 흐름의 변화는 다음과 같다.

- 회원가입 시 비밀번호는 BCrypt 해시로만 저장된다.
- 로그인은 `matches()`로 검증하므로 평문 저장/평문 비교가 사라졌다.
- 게시글 작성자는 body의 `userId`가 아니라 JWT에서 검증된 `memberId`로 결정된다.
- 게시글 수정/삭제는 작성자 본인만 가능하고, 작성자가 아니면 `403 AUTH_003`을 반환한다.
- 좋아요는 `(post_id, user_id)` 유니크 구조로 중복 추가를 막고, 취소도 멱등하게 처리한다.
- 로그아웃하면 Refresh Token을 삭제하고 현재 Access Token을 블랙리스트에 저장한다.
- 블랙리스트 처리된 Access Token은 만료 전이어도 보호 API에서 401로 차단된다.
- Kakao/Google 로그인은 외부 provider 토큰으로 사용자 정보를 조회한 뒤, 우리 서버의 JWT를 새로 발급한다.
